 ...
 * - R4 i18n Strict: Utiliza window.LuxI18n.t() para resolución de textos.
...
class AtAdminGate extends HTMLElement {
    ...
    _setupManualFlow() {
        ...
        const submitHandler = (e) => {
            ...
            if (!this._validateEmail(email)) {
                this._showError(window.LuxI18n ?
                    window.LuxI18n.t('errors.invalid_email', 'Formato de correo inválido') :
    ...
    async _handleOAuth(providerName, providerId) {
        ...
        try {
            ...
            // Construir objeto de sesión agnóstico (R0)
     ...
     */
    _buildSession(firebaseUser, provider) {
        return {
            ...
            provider: provider,
            token: null, // Se obtiene asíncronamente vía getIdToken si es necesario
            tenantValidated: false, // PassportEngine validará esto
            claims: {}, // Se población tras getIdTokenResult
            authenticated: true,
            timestamp: new Date().toISOString()
        };
    }
    ...
    _resolveErrorMessage(error) {
        ...
        if (window.LuxI18n && window.LuxI18n.t) {
            return window.LuxI18n.t(key, fallback);
        }
        return fallback;
    }
